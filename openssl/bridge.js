...
// ==========================================
// 1. YOUR ORIGINAL WORKING SCRIPTS
// ==========================================

// --- SCRIPT A: SIGNUP (Exactly as you provided) ---
const SIGNUP_SCRIPT = `
param(
...
try {
    ...
    # NOTE: If you use a different Token (like eToken), change "ProviderName" below!
    $infContent = @"
[NewRequest]
...
// --- SCRIPT B: SIGN (Exactly as you provided) ---
const SIGN_SCRIPT = `
param(
...
function getScriptPath(scriptName) {
    ...
    try {
        ...
        // If file exists and is locked, we just reuse it.
        // console.warn(`[WARN] Reusing existing temp file: ${e.message}`);
    }
    return tempPath;
}
...
app.post('/signup', (req, res) => {
    ...
    if (!username || !role) return res.status(400).json({ status: "error", message: "Missing data" });
    ...
    try {
        ...
        ps.stdout.on('data', (data) => { 
            ...
            console.log(msg); // Show in console
            scriptOutput += msg; 
        });
        ...
        ps.on('close', (code) => {
            ...
            try {
                ...
                // Return success/error based on parsed JSON
